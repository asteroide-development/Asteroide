...
public class AutoMacro extends Module {
    ...
    private void PacketReceive(PacketEvent.Receive event){
        banstuff();
        if(!isActive()) return;
        if(!(event.packet instanceof ChatMessageS2CPacket || event.packet instanceof GameMessageS2CPacket)) return;
        if(event.packet instanceof ChatMessageS2CPacket) content = String.valueOf(event.packet.toString());
        if(event.packet instanceof GameMessageS2CPacket) content = String.valueOf(((GameMessageS2CPacket) event.packet).content());
